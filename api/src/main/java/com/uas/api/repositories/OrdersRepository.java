...
public interface OrdersRepository extends JpaRepository<Orders, Integer> {
      ...
      */
     @Query(value = "SELECT * FROM Orders WHERE LocationName=:location AND SupplierEmail=:supplierEmail ORDER BY OrderID DESC LIMIT 1", nativeQuery = true)
     Orders findByAttributes(@Param("location") String location, @Param("supplierEmail") String supplierEmail);
}
